-- GreenLedger — Phase A schema, part 4: grants (the enforced trust boundary)
--
...
GRANT USAGE ON SCHEMA public TO greenledger_app, greenledger_ai;

-- --- greenledger_app: the service layer -----------------------------------------
-- Append-only on the journal + reference tables.
GRANT SELECT, INSERT ON accounts, source_documents, journal_entries,
                        journal_lines, audit_log, orgs                TO greenledger_app;
-- Periods can be opened and transitioned (open -> closed -> locked).
GRANT SELECT, INSERT, UPDATE ON periods                              TO greenledger_app;
-- The app READS AI proposals but may never write them.
GRANT SELECT ON proposals                                            TO greenledger_app;
...
REVOKE UPDATE, DELETE ON journal_entries FROM greenledger_app;
REVOKE UPDATE, DELETE ON journal_lines   FROM greenledger_app;

-- --- greenledger_ai: the categorizer --------------------------------------------
...
GRANT SELECT, INSERT ON proposals                                   TO greenledger_ai;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public               TO greenledger_ai;
-- Read-only context so it can categorize (chart of accounts + the raw docs).
GRANT SELECT ON accounts, source_documents                          TO greenledger_ai;
