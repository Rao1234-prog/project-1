...
/* ============================================================
   GREENLEDGER — AI-native finance function with a paper trail.
...
export default function App() {
  ...
  async function loadAll(orgId) {
    const [b, e, q, cl, tb, vc, ac] = await Promise.all([
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.paper, color: T.ink, fontFamily: T.sans }}>
      ...
      <header style={{ background: T.greenDeep, color: T.paper, padding: "14px 28px",
        ...
        <div style={{ fontFamily: T.serif, fontSize: 24, fontWeight: 600, letterSpacing: "-0.01em" }}>GreenLedger</div>
        ...
        {!loading && tab === "advisor" && (
          <div>
            ...
            <div style={{ marginTop: 14, fontSize: 11.5, fontFamily: T.mono, color: T.brass }}>
              ADVISORY — generated by greenledger-cfo-1, reviewed before delivery. No action taken automatically.
            </div>
          </div>
        )}

        {/* ================= AUDIT ================= */}
        {!loading && tab === "audit" && accuracy && (
          <div>
            <SectionTitle k="Categorization accuracy audit" t="Proposals vs. human decisions"
              sub="How often the AI's proposed account matched what a human approved. Corrections tighten future autonomy; nothing here can auto-post a novel vendor." />
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 14, marginBottom: 22 }}>
              <Stat label="First-pass agreement" v={accuracy.overall_first_pass_agreement == null ? "—" : `${Math.round(accuracy.overall_first_pass_agreement * 100)}%`}
                note={`gate ${Math.round(accuracy.gate * 100)}% · ${accuracy.meets_gate ? "met" : "below"}`}
                noteColor={accuracy.meets_gate ? T.green : T.brass} big />
              <Stat label="Reviewed decisions" v={String(accuracy.reviewed_count)} note="human approve/correct/reject" />
              <Stat label="Wrong auto-posts" v={String(accuracy.wrong_auto_posts)} note="must be 0 — structural"
                noteColor={accuracy.wrong_auto_posts === 0 ? T.green : T.red} />
            </div>

            <Card title="Agreement by proposed account">
              {accuracy.per_account.length === 0 ? <div style={{ color: T.inkSoft }}>No reviewed decisions yet.</div> : (
                <table className="ruled" style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr style={{ textAlign: "left", color: T.inkSoft }}>
                    {["Account", "Agreement", "Agreements", "Total"].map(h =>
                      <th key={h} style={{ padding: "6px 10px", fontSize: 11, textTransform: "uppercase", letterSpacing: ".05em" }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {accuracy.per_account.map(a => (
                      <tr key={a.account_code}>
                        <td style={{ padding: "6px 10px", fontFamily: T.mono }}>{a.account_code} {bal[a.account_code]?.name || ""}</td>
                        <td style={{ padding: "6px 10px", fontFamily: T.mono }}>{a.agreement == null ? "—" : `${Math.round(a.agreement * 100)}%`}</td>
                        <td style={{ padding: "6px 10px", fontFamily: T.mono }}>{a.agreements}</td>
                        <td style={{ padding: "6px 10px", fontFamily: T.mono }}>{a.total}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <Card title="Agreement by pattern type" style={{ marginTop: 14 }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 22, fontFamily: T.mono, fontSize: 13 }}>
                {accuracy.per_pattern.length === 0 ? <span style={{ color: T.inkSoft }}>No data yet.</span> :
                  accuracy.per_pattern.map(p => (
                    <span key={p.pattern_match}>
                      <b>{p.pattern_match}</b>: {p.agreement == null ? "—" : `${Math.round(p.agreement * 100)}%`}
                      <span style={{ color: T.inkSoft }}> ({p.agreements}/{p.total})</span>
                    </span>
                  ))}
              </div>
              <p style={{ fontSize: 12, color: T.inkSoft, marginTop: 12, lineHeight: 1.5 }}>{accuracy.wrong_auto_posts_note}</p>
            </Card>
          </div>
        )}
      </main>
